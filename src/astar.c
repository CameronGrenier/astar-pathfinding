...
PathNode* a_star(Node *start_node) {
  ...
  while (open_set.size != 0) {
    ...
    for (int i = 0; i < 4; i++) {
      Node *n = neighbours[i];
      if (n == NULL) {continue;}
      if (closed_set[n->pos.y * cols + n->pos.x] == 1) {continue;}
      if (shared_map[n->pos.y][n->pos.x].type == OBSTACLE) {continue;}
      if (shared_map[n->pos.y][n->pos.x].type == AGENT && !nodes_equal(n, goal_node)) {continue;}
      int g = curr.g + 1;
